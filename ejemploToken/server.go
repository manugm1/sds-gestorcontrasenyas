...
//*jwt.Token
func setToken(userEmail string) string{
		...
		if usuarios[usuario.Email].Email == usuario.Email {
         ...
         token := setToken(usuario.Email)
				...
				sesion := Sesion{Email: usuario.Email, TiempoLimite: time.Now().Add(time.Hour * time.Duration(0) +
                                 ...
                                 time.Second * time.Duration(30)), Token: token }
				sesiones[usuario.Email] = sesion
				r = Resp{Ok: true, Msg: "El usuario se ha logueado correctamente.", Token: token }    // formateamos respuesta
			}else{
				r = Resp{Ok: false, Msg: "La contraseña no es correcta. Vuelva a intentarlo.", Token: ""}    // formateamos respuesta
			}
	  }else{
			r = Resp{Ok: false, Msg: "No coinciden los parámetros del usuario. Vuelve a intentarlo.", Token: ""}    // formateamos respuesta
		}
	}else{
		r = Resp{Ok: false, Msg: "El usuario no existe, regístrate y vuelve a intentarlo.", Token: ""}    // formateamos respuesta
	}
	comunicarCliente(w, r)
}
...
func crearEntrada(w http.ResponseWriter, request *http.Request){
  ...
  token := request.Form.Get("token")
	...
	if esEmailLogueado(usuario.Email) &&  sesiones[usuario.Email].Token==token{
